"""
Math grading environment for RL2.
Includes grading utilities from tinker-cookbook.
"""
import contextlib
import logging
import math
...
# ======================================================================
# Grading with timeout
# ======================================================================


def safe_grade(given_answer: str, ground_truth: str, grader: str = "sympy", timeout: float = 1.0) -> bool:
...
def check_answer(sample_str: str, ground_truth: str, grader: str = "sympy", timeout: float = 1.0) -> bool:
    """
    Check if the answer in the sample is correct.
    Extracts boxed answer and grades it against ground truth.
    """
    try:
        answer = extract_boxed(sample_str)
    except ValueError:
        return False
    return safe_grade(answer, ground_truth, grader, timeout)


# ======================================================================
# RL2 Environment Step Function
# ======================================================================
...
async def step(state, action, extra_info):
    ...
    Args:
        state: Current state (unused for single-turn)
        action: Generated response text
        extra_info: Dict containing 'answer' (ground truth) and optionally 'grader', 'timeout'
    
    Returns:
        Dict with next_state, reward, score, done, extra_info
    """
    try:
        ground_truth = extra_info["answer"]
        grader = extra_info.get("grader", "sympy")
        timeout = extra_info.get("timeout", 1.0)
        
        # Check if answer is correct
        is_correct = check_answer(action, ground_truth, grader, timeout)
        reward = float(is_correct)
    except Exception as e:
        logger.warning(f"Error grading answer: {e}")
        reward = 0.0
