...
torchrun \
    --nproc_per_node=4 \
    -m RL2.trainer.offline_rl \
    data.path=simonycl/gsm8k_training_negative_multi_turn_1k_${DATASET_MODEL}_transformed \
    data.max_length=8192 \
    data.batch_size=64 \
    actor.model_name=meta-llama/Llama-3.2-1B-Instruct \
    actor.sp_size=4 \
    ...
    offline_rl.negative_label_scale=${NEGATIVE_LABEL_SCALE} \
    trainer.project=synthetic_gsm \
    trainer.experiment_name=llama-3.2-1b-multi-turn-inst-offline-rl \
    trainer.n_epochs=3
...
torchrun \
    --nproc_per_node=4 \
    -m RL2.trainer.offline_rl \
    data.path=simonycl/gsm8k_training_negative_chain_of_thought_1k_${DATASET_MODEL}_transformed \
    data.max_length=8192 \
    data.batch_size=64 \
    actor.model_name=meta-llama/Llama-3.2-1B-Instruct \
    actor.sp_size=4 \
    ...
    offline_rl.negative_label_scale=${NEGATIVE_LABEL_SCALE} \
    trainer.project=synthetic_gsm \
    trainer.experiment_name=llama-3.2-1b-chain-of-thought-inst-offline-rl \
    trainer.n_epochs=3
...
MODELS=(
    ...
    "ckpts/llama-3.2-1b-multi-turn-inst-offline-rl"
    "ckpts/llama-3.2-1b-chain-of-thought-inst-offline-rl"
)
